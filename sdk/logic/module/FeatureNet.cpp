 ...
 */
void FeatureNet::targetInit(int index, double reid_threshold)
{
    reid_threshold_ = reid_threshold;
    std::vector<double> full_feature = g_xbotgoSingleStrategyCallbacks->onGetFeature(index);
    target_feature_base_.push_back(full_feature);
    previous_target_feature_ = full_feature; // 初始化时取第一个目标的特征作为上一帧的目标特征
    target_feature_list_full_.push_back(full_feature);

    // pose_ = g_xbotgoSingleStrategyCallbacks->onGetPose(index);

    // std::cout << "FeatureNet::track init index="<<index<<std::endl;
    // std::cout << "FeatureNet::track init pose="<<pose_<<std::endl;

    // if (pose_ == pose_full_) {
    // std::vector<double> full_feature = g_xbotgoSingleStrategyCallbacks->onGetFeature(index);
    // std::vector<double> half_feature = g_xbotgoSingleStrategyCallbacks->onGetHalfFeature(index);

    // std::cout << "FeatureNet::track init pose is full, full_feature="<<full_feature.size()<<std::endl;
    // std::cout << "FeatureNet::track init pose is full, half_feature="<<half_feature.size()<<std::endl;

    // current_target_feature_ = full_feature;
    // target_feature_base_.push_back(full_feature);
    // previous_target_feature_ = full_feature; // 初始化时取第一个目标的特征作为上一帧的目标特征

    // target_feature_list_full_.push_back(full_feature);
    // target_feature_list_half_.push_back(half_feature);
    // } else {

    // std::vector<double> half_feature = g_xbotgoSingleStrategyCallbacks->onGetFeature(index);

    // std::cout << "FeatureNet::track init pose is half, half_feature="<<half_feature.size()<<std::endl;

    // current_target_feature_ = half_feature;
    // target_feature_base_.push_back(half_feature);
    // previous_target_feature_ = half_feature; // 初始化时取第一个目标的特征作为上一帧的目标特征

    // target_feature_list_half_.push_back(half_feature);
    // }

    // std::cout << "FeatureNet::track init target_feature_base_="<<target_feature_base_.size()<<std::endl;
    // std::cout << "FeatureNet::track init target_feature_list_full_="<<target_feature_list_full_.size()<<std::endl;
    // std::cout << "FeatureNet::track init target_feature_list_half_="<<target_feature_list_half_.size()<<std::endl;
}
 ...
 */
int FeatureNet::reidTrack()
{
    // 调用API获取当前帧所有人的特征
    current_frame_features_ = g_xbotgoSingleStrategyCallbacks->onGetAllFeatures();
    // std::cout << "FeatureNet::reidTrack, current_frame_features_="<<current_frame_features_.size()<<std::endl;

    // 分别从基础、全身、半身特征队列中查找
    int index = searchTarget(target_feature_base_, reid_base_);
    if (index != -1)
        return index;

    index = searchTarget(target_feature_list_full_, reid_full_);
    if (index != -1)
        return index;

    // index = searchTarget(target_feature_list_half_, reid_half_);
    // if (index != -1)
    //     return index;
 ...
 */
int FeatureNet::searchTarget(const std::vector<std::vector<double>>& feature_list, int debug_log_state)
{

    std::cout << "FeatureNet::search target debug_log_state" << debug_log_state << std::endl;

    for (size_t i = 0; i < feature_list.size(); i++) {
        std::cout << "FeatureNet::search target i=" << i << ",size=" << feature_list[i].size() << std::endl;
    }

    std::cout << "FeatureNet::search target feature_list =" << feature_list.size() << std::endl;
    std::cout << "FeatureNet::search target current_frame_features_ =" << current_frame_features_.size() << std::endl;
    ...
    if (*min_distance < reid_threshold_) {
        ...
        debug_log_distance_half_ = (debug_log_state == reid_half_ || debug_log_state == reid_base_) ? *min_distance : 0.0;
        // ------------------------ Debug Log ------------------------

        previous_target_feature_ = current_frame_features_[min_index];
        return min_index;
    }
    return -1;
}
 ...
 * @param index 目标的索引
 */
void FeatureNet::getFeature(const int index)
{
    current_target_feature_ = g_xbotgoSingleStrategyCallbacks->onGetFeature(index);
    // std::cout << "FeatureNet::getFeature ="<<current_target_feature_.size()<<std::endl;
}

/**
 * @brief 更新上一帧的目标特征
 */
void FeatureNet::updatePreviousTargetFeature()
{
 ...
 * @return false 验证失败
 */
bool FeatureNet::reidLocalVerification()
{
 ...
 * @return false 更新失败
 */
bool FeatureNet::updateFeatureList(std::vector<std::vector<double>>& target_feature_list, double& debug_log_distance, const double threshold)
{
    double distance = 0.0;
 ...
 */
void FeatureNet::updateTargetFeatureList(const int index)
{
    // 调用API获取姿态pose
    pose_ = g_xbotgoSingleStrategyCallbacks->onGetPose(index);
    ...
    if (pose_ == pose_full_) {
        updateFeatureList(target_feature_list_full_, debug_log_distance_full_, feature_list_full_threshold_);
        ...
        //     std::cout << "FeatureNet::updateTargetFeatureList onGetHalfFeature="<<current_target_feature_.size()<<std::endl;
        //     updateFeatureList(target_feature_list_half_, debug_log_distance_half_, feature_list_half_threshold_);
        // }
    } else if (pose_ == pose_half_) {
 ...
 * @return double 欧氏距离
 */
double FeatureNet::calculateDistance(const std::vector<double> feature1, const std::vector<double> feature2)
{
 ...
 */
std::vector<double> FeatureNet::calculateMinimalDistances(const std::vector<std::vector<double>> feature_list1, const std::vector<std::vector<double>> feature_list2)
{
 ...
 * @return false 如果不重叠
 */
bool isOverLapping(const std::vector<YOLODetectionResult>& yolo_detection_results, int index, float scale)
{
