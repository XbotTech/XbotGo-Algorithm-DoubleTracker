...
typedef std::vector<std::vector<std::vector<double>>> Feature3D;
 ...
 * @return false 如果不重叠
 */
bool isOverLapping_(const std::vector<YOLODetectionResult>& yolo_detection_results, int index, float scale = 1.0);

/**
 * @brief 特征向量网络类，用于目标特征的提取、保存和Reid单目标追踪
 */
class DoubleFeatureNet
{
public:
    /**
     * @brief 构造函数
     ...
     */
    void targetInit(std::vector<int> indexs, double reid_threshold);
     ...
     */
    int reidTrack(int id);

    /**
     * @brief Reid局部验证——
     * @brief 将当前成功追踪的目标特征和保存的上一帧追踪目标特征进行比较，如果距离大于设定的欧式距离阈值，验证失败，否则验证成功
     * @param id 表示表示第几个目标 0 表示 第一个目标 1 表示第二个目标
     *
     * @return true 验证成功
     * @return false 验证失败
     */
    bool reidLocalVerification(const int id);
     ...
     */
    void updateTargetFeatureList(const int id, const int index);

    /**
     * @brief 调用API获取指定索引的目标特征
     * @param 表示第几个目标 0 表示 第一个目标 1 表示第二个目标
     * @param index 目标的索引
     */
    void getFeature(const int id, const int index);
...
private:
     ...
     */
    int searchTarget(const std::vector<std::vector<double>>& feature_list, const int id, int state);
     ...
     * @return false 更新失败
     */
    bool updateFeatureList(const int id, std::vector<std::vector<double>>& target_feature_list, double& feature_extractor_distance, const double threshold);

private:
    int pose_;                                                 /**< 当前追踪到的对象的姿态 */
    std::vector<std::vector<double>> previous_target_feature_; /**< 上一帧的目标特征, 0位置表示第一个目标，1位置表示第二个目标 */
    std::vector<std::vector<double>> current_target_feature_;  /**< 当前帧的目标特征 */
    ...
    Feature3D target_feature_list_full_ = Feature3D(2);        /**目标特征的全身列表，0位置表示第一个目标的特征队列 */
...
public:
    ...
    double debug_log_distance_full_; /**< 调试日志变量：与全身队列的最小欧式距离 */
    double debug_log_distance_half_; /**< 调试日志变量：与半身队列的最小欧式距离 */
    double debug_log_distance_pre_;  /**< 调试日志变量：与上一帧目标的欧式距离 */
};
} // namespace XbotgoSDK
