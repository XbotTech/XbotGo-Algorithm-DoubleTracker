 ...
 */
DoubleSOT::DoubleSOT(const int& frameRate, const int& trackBuffer, const float& trackThresh, const float& highThresh, const float& matchThresh)
 ...
 */
void DoubleSOT::targetInit(const std::vector<YOLODetectionResult>& detectionResults, std::vector<int> indexs)
{
    delete _byte;
    ...
    _targetId.resize(2, -1);
    _targetId[0] = outputs[indexs[0]].track_id;
    _targetId[1] = outputs[indexs[1]].track_id;
}
 ...
 */
int DoubleSOT::track(const std::vector<YOLODetectionResult>& detectionResults, int id)
{
    ...
    for (const auto& output : outputs) {
        if (_targetId[id] == output.track_id) {
 ...
 * @param index 追踪目标的YOLO检测结果索引(即追踪目标是YOLO检测结果中的第几个)
 */
void DoubleSOT::restart(const std::vector<YOLODetectionResult>& detectionResults, std::vector<int> indexs)
{
    targetInit(detectionResults, indexs);
}
