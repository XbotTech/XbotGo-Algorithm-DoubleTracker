...
public:
     ...
     */
    DoubleSOT(const int& frame_rate = 30, const int& track_buffer = 30,
     ...
     */
    void targetInit(const std::vector<YOLODetectionResult>& detectionResults, std::vector<int> indexs);
     ...
     */
    int track(const std::vector<YOLODetectionResult>& detectionResults, int id);
     ...
     * @param index 追踪目标的YOLO检测结果索引(即追踪目标是YOLO检测结果中的第几个)
     */
    void restart(const std::vector<YOLODetectionResult>& detectionResults, std::vector<int> indexs);
